...
{
    qWarning()<<"seat_capabilities:"<<caps<<" mTouch:"<<mTouch;
    mCaps = caps;
    ...
    if (caps & WL_SEAT_CAPABILITY_POINTER && !mPointer) {
        ...
        if (mQDisplay->pointerGesture()) {
            if (!mPointerTouchDevice) {
                mPointerTouchDevice = new QTouchDevice;
                mPointerTouchDevice->setType(QTouchDevice::TouchScreen);
                mPointerTouchDevice->setCapabilities(QTouchDevice::Position);
                QWindowSystemInterface::registerTouchDevice(mPointerTouchDevice);
            }

            QWaylandPointerPinchGesture *pinch = new QWaylandPointerPinchGesture(mQDisplay, mPointerTouchDevice, mQDisplay->pointerGesture()->get_pinch_gesture(mPointer->wl_pointer()));
            QWaylandPointerSwipeGesture *swipe = new QWaylandPointerSwipeGesture(mQDisplay, mPointerTouchDevice, mQDisplay->pointerGesture()->get_swipe_gesture(mPointer->wl_pointer()));
        }
...
void QWaylandInputDevice::Pointer::pointer_axis(uint32_t time, uint32_t axis, int32_t value)
{
    ...
    case WL_POINTER_AXIS_VERTICAL_SCROLL:
        mFrameData.delta.ry() += wl_fixed_to_double(value * 5);
        qCDebug(lcQpaWaylandInput) << "wl_pointer.axis vertical:" << mFrameData.delta.y();
        break;
    case WL_POINTER_AXIS_HORIZONTAL_SCROLL:
        mFrameData.delta.rx() += wl_fixed_to_double(value * 5);
...
void QWaylandInputDevice::handleTouchPoint(int id, Qt::TouchPointState state, const QPointF &surfacePosition)
{
    ...
    if (state == Qt::TouchPointMoved || state == Qt::TouchPointPressed) {
        ...
        QPointF globalPosition = QHighDpi::toNativeLocalPosition(win->window()->mapToGlobal(QHighDpi::fromNativeLocalPosition(localPosition.toPoint(), win->window())), win->window()) + delta;
...
void QWaylandInputDevice::Touch::touch_frame()
{
    ...
    QWindowSystemInterface::handleTouchEvent(window, motionTime,mParent->mTouchDevice, mPendingTouchPoints);
